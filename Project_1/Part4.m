...
%% 5a) Twin ground pads (coplanar layout from paper: wg = 11.5 mm, lg = 10 mm)
% Left pad: from x = 0 to x = wg
leftPad = antenna.Rectangle('Length', wg, 'Width', lg, ...
            'Center', [wg/2 , lg/2]);

% Right pad: from x = boardW - wg to x = boardW
rightPad = antenna.Rectangle('Length', wg, 'Width', lg, ...
            'Center', [boardW - wg/2 , lg/2]);

groundPads = leftPad + rightPad;


%% 5b) Monopole strip (from top of ground up)
vMono = antenna.Rectangle('Center',[boardW/2, monoLen / 2], ...
                         'Length',monoW,'Width',monoLen);
hMono = antenna.Rectangle('Center',[boardW/2 - monoW / 2 + km/2 , lg + lm - monoW / 2], ...
                          'Length', km, 'Width', monoW);
mono = vMono + hMono;

figure('Name','L-shaped Monopole + Ground Pads');
show(groundPads + mono); axis equal; view(0,90);
title('L-shaped monopole and twin ground pads');
